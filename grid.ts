...
interface RowCol { _row: HiddenList<Row>; _col: HiddenList<Col>; }
interface Point { x: number; y: number; }
interface HasIndex { _index: number; }
interface SortParams { _header: string; _ascending: boolean; }

interface Data {
	_data: any;
	_headers: string[];
	_markDirty: () => void;
	_runAfterChange: () => void;
	gridParams?: any;
}
...
interface Selection {
	_mode?: string;
	_minRow: HiddenList<Row>;
	_maxRow: HiddenList<Row>;
	_minCol: HiddenList<Col>;
	_maxCol: HiddenList<Col>;
}
...
class Col {
	...
	_formula: string;
	_formulaObject: Function; // (i: number) => any
	
	_format: string;
	_formatObject: any; // we don't have to use this, because sprintf memoizes the parse trees
	...
	_calculate(): void {
		...
		if (col._visited) { throw new Error('circular reference at column "' + col._header + '"'); }
		col._visited = true;
		
		// calculate uncalculated srcs first
		col._srcs.forEach(function(src) { if (!src._calculated) { src._calculate(); } });
	...
	_setFormula(formula: string): void {
...
class Style {
	...
	_border: any;
	_hMargin: number; // this is the margin between cell border and cell text
	_vMargin: number;
...
class GridLinkedList<T> {
	...
	_data: T;
	_prev: GridLinkedList<T>;
	_next: GridLinkedList<T>;
	
	constructor() {
		this._prev = this;
		this._next = this;
	}
	_add(data: T): GridLinkedList<T> {
...
class HiddenList<T> {
	...
	_prev: HiddenList<T>;
	_next: HiddenList<T>;
	_visibleNext: HiddenList<T>;
	_visiblePrev: HiddenList<T>;
	...
	_add(data: T, visible: boolean): HiddenList<T> {
	...
	_hideUntil(that: HiddenList<T>) {
		this._visibleNext = that;
		that._visiblePrev = this;
	}
	_showUntil(that: HiddenList<T>) {
...
export class Grid {
	...
	_defaultCellStroke: string;
	_defaultHeaderStroke: string;
	_selectedCellStroke: string;
	_selectedHeaderStroke: string;
	_defaultCellFill: string;
	_defaultHeaderFill: string;
	_selectedCellFill: string;
	_selectedHeaderFill: string;
	...
	_rows: HiddenList<Row>;
	_cols: HiddenList<Col>;
	...
	_visibleRows: HiddenList<Row>[];
	_visibleCols: HiddenList<Col>[];
	...
	_dataComponent: Data;
	_columnParams: any; // ColumnParams[]
	
	_editMode: string; // value, formula, format, style, filter
	
	// to write, with columns
	_filter: string;
	_sort: string; // i'm not sure what parts of the code agree with this, but _sort should be a custom function - single column asc/dsc sorts can be stored in _multisort
	_multisort: GridLinkedList<SortParams>;
	_multisortIndicatorDict: any; // string -> number (col header -> index)
	
	_filterFn: any; // (x: any) => boolean
	_sortFn: any; // (a: any, b: any) => number
	...
	constructor(dataComponent: Data, div: HTMLDivElement) {
		...
		if (!gridJson.columns) { gridJson.columns = dataComponent._headers.map(function(header) { return {header:header,visible:true,width:64,formula:'',format:null,style:null}; }); }
		...
		Object.defineProperty(this, 'data', { 
			...
			set : function(value) {
				...
				if (grid._dataComponent._markDirty) { grid._dataComponent._markDirty(); }
				grid._resetData();
			}
		});
		...
		grid._multisort = new GridLinkedList<SortParams>();
	...
	_resetData(): void {
		...
		grid._rows = new HiddenList<Row>();
		grid._cols = new HiddenList<Col>();
		
		for (var i = 0; i < grid._dataComponent._data.length; i++) { grid._rows._add(new Row(i, grid._dataComponent._data[i]), true); }
		...
		if (grid._multisort._next !== grid._multisort) { grid._setMultisort(); }
		if (grid._filter !== null) { grid._setFilter(grid._filter); }
	...
	_writeData(format: string): string {
	...
	_draw(): void {
		...
		for (var i = 1; i < grid._visibleRows.length; i++)
		{
			...
			if (i == 1) { clipTp = bt - grid._rowHeight; }
			if (i == grid._visibleRows.length - 1) { clipHg = grid._rowHeight; }
		...
		for (var j = 1; j < grid._visibleCols.length; j++)
		{
			...
			if (j == 1) { clipLf = rt - col._width; }
			if (j == grid._visibleCols.length - 1) { clipWd = col._width; }
		...
		if (grid._selected)
		{
			...
			else
			{
				for (var i = 1; i < grid._visibleRows.length; i++)
				{
					if (grid._visibleRows[i] == sel._minRow) { sy0 = grid._ys[i+0]; }
					if (grid._visibleRows[i] == sel._maxRow) { sy1 = grid._ys[i+1]; break; }
				}
			...
			else
			{
				for (var j = 1; j < grid._visibleCols.length; j++)
				{
					if (grid._visibleCols[j] == sel._minCol) { sx0 = grid._xs[j+0]; }
					if (grid._visibleCols[j] == sel._maxCol) { sx1 = grid._xs[j+1]; break; }
				}
	...
	_pointToRowCol(x: number, y: number): RowCol {
		...
		for (var i = 1; i < grid._ys.length; i++) { if (y <= grid._ys[i]) { row = grid._visibleRows[i-1]; break; } }
		for (var j = 1; j < grid._xs.length; j++) { if (x <= grid._xs[j]) { col = grid._visibleCols[j-1]; break; } }
	...
	_calculate(): void {
		...
		var grid = this;
		grid._cols._enumerate().forEach(function(col) { if (!col._calculated) { col._calculate(); } });
	}
	
	_setMouseHandles(): void {
		...
		canvas.onmousemove = function(mouseMoveEvent) {
			...
			if (x0 < m.x && m.x < xn && y0 < m.y && m.y < yn)
			{
				...
				canvas.onmousedown = function(mouseDownEvent) {
					...
					if (target._row == null && target._col == null) { return; } // cannot select top-left cell
					...
					if (mouseDownEvent.button == 0)
					{
						canvas.onmousemove = function(mouseDragEvent) {
							...
							if (pointedRowCol._row === null || pointedRowCol._col === null) { return; }
	...
	_setKeyHandles(): void {
		...
		canvas.onkeydown = function(e) {
			...
			else if (key == 33 || key == 34) // page up/down
			{
				...
				if (key == 33) // page up
				{
					for (var i = 0; i < n; i++)
					{
						...
						if (grid._cursor._row == grid._rows) { grid._cursor._row = grid._rows._visibleNext; break; }
					}
				}
				else // page down
				{
					for (var i = 0; i < n; i++)
					{
						...
						if (grid._cursor._row == grid._rows) { grid._cursor._row = grid._rows._visiblePrev; break; }
					}
				}
				
				if (e.shiftKey) { grid._selectRange(); } else { grid._selectCell(); }
			}
			...
			{
				if (grid._selected._minRow == null && grid._selected._maxRow == null) // col header selected
				{
					...
					else // Arrow or Shift+Arrow - move cursor like normal
					{
						if (key == 37 || key == 39)
						{
							if (key == 37) // left
							{
								if (grid._cursor._col._visiblePrev != grid._cols) { grid._cursor._col = grid._cursor._col._visiblePrev; }
							}
							else if (key == 39) // right
							{
								if (grid._cursor._col._visibleNext != grid._cols) { grid._cursor._col = grid._cursor._col._visibleNext; }
							}
				...
				else if (grid._selected._minCol == null && grid._selected._maxCol == null) // row header selected
				{
					...
					else // Arrow or Shift+Arrow - move cursor like normal
					{
						if (key == 38 || key == 40)
						{
							if (key == 38) // up
							{
								if (grid._cursor._row._visiblePrev != grid._rows) { grid._cursor._row = grid._cursor._row._visiblePrev; }
							}
							else if (key == 40) // down
							{
								if (grid._cursor._row._visibleNext != grid._rows) { grid._cursor._row = grid._cursor._row._visibleNext; }
							}
		...
		else
		{
			...
			while (dstRow != grid._selected._maxRow._visibleNext)
			{
				...
				while (dstCol != grid._selected._maxCol._visibleNext)
				{
					...
					if (srcCol == grid._copied._maxCol._visibleNext) { srcCol = grid._copied._minCol; } // toroidal wraparound
				}
				
				srcRow = srcRow._visibleNext;
				dstRow = dstRow._visibleNext;
				
				if (srcRow == grid._copied._maxRow._visibleNext) { srcRow = grid._copied._minRow; } // toroidal wraparound
			}
		}
	...
	_scrollBy(offset: number, rows: boolean): void {
	...
	_resizeRow(oldsize: number, newsize: number) {
	...
	_resizeCol(col: Col, oldsize: number, newsize: number): void {
	...
	_beginEditArray(format: string): void {
	...
	_acceptEdit(): void {
		...
		{
			if (col._data._header == str) { return; } // no change, no need to do anything
	...
	_selectCell(): void {
		...
		// aria
		var row = grid._cursor._row._data;
		var col = grid._cursor._col._data;
		var addr = col._header + ' ' + row._index.toString();
		var text = Format(row._object[col._header], col._format);
	...
	_selectRange(): void {
		...
		// aria
		var minrow = grid._selected._minRow._data;
		var mincol = grid._selected._minCol._data;
		var maxrow = grid._selected._maxRow._data;
		var maxcol = grid._selected._maxCol._data;
		var range = mincol._header + minrow._index.toString() + ' to ' + maxcol._header + maxrow._index.toString();
		grid._ctx.canvas.setAttribute('aria-label', range);
	...
	_calcMaxRowFromMinRow(): void {
		...
		while (y < grid._bt)
		{
			y += grid._rowHeight;
			rowElt = rowElt._visibleNext;
			if (rowElt == grid._rows) { rowElt = rowElt._visiblePrev; break; }
		}
		
		grid._scroll._maxRow = rowElt;
	}
	_calcMaxColFromMinCol(): void {
		...
		while (x < grid._rt)
		{
			x += colElt._data._width;
			colElt = colElt._visibleNext;
			if (colElt == grid._cols) { colElt = colElt._visiblePrev; break; }
		}
		
		grid._scroll._maxCol = colElt;
	}
	_calcMinRowFromMaxRow(): number {
		...
		while (y > (grid._tp + grid._rowHeight))
		{
			...
			if (rowElt == grid._rows) { rowElt = rowElt._visibleNext; break; }
		}
	...
	_calcMinColFromMaxCol(): number {
		...
		while (x > (grid._lf + grid._rowHeaderWidth))
		{
			...
			if (colElt == grid._cols) { colElt = colElt._visibleNext; break; }
		}
	...
	_setRange(value: any): void {
		
		var grid = this;
		grid._setRangeGeneral(value, grid._selected);
	}
	_setRangeGeneral(value: any, sel: Selection): void {
	...
	_insertRowsAbove(): void { var grid = this; grid._insertRows(true); }
	_insertRowsBelow(): void { var grid = this; grid._insertRows(false); }
	_insertColsLeft(): void { var grid = this; grid._insertCols(true); }
	_insertColsRight(): void { var grid = this; grid._insertCols(false); }
	_moveRowsUp(): void { var grid = this; grid._moveRows(-1); }
	_moveRowsDown(): void { var grid = this; grid._moveRows(1); }
	_moveColsLeft(): void { var grid = this; grid._moveCols(-1); }
	_moveColsRight(): void { var grid = this; grid._moveCols(1); }
	_insertRows(bAbove: boolean): void {
		
		var grid = this;
		
		var sel = grid._selected;
		
		var n = CountVisible<Row>(sel._minRow, sel._maxRow);
	...
	_insertCols(bLeft: boolean): void {
		
		var grid = this;
		
		var sel = grid._selected;
		
		var n = CountVisible<Col>(sel._minCol, sel._maxCol);
	...
	_deleteRows(): void {
		...
		if (sel._minRow._visiblePrev == grid._rows && sel._maxRow._visibleNext == grid._rows) { return; }
		...
		if (remaining == grid._rows) { remaining = remaining._visibleNext; }
	...
	_deleteCols(): void {
		...
		if (sel._minCol._visiblePrev == grid._cols && sel._maxCol._visibleNext == grid._cols) { return; }
		...
		if (remaining == grid._cols) { remaining = remaining._visibleNext; }
	...
	_moveRows(k: number): void {
		...
		if (k < 0 && min._visiblePrev == sentinel) { return; }
		if (k > 0 && max._visibleNext == sentinel) { return; }
	...
	_showRows(): void {
		...
		if (min == max)
		{
			if (min._visiblePrev == grid._rows) { min = grid._rows._next; }
			if (max._visibleNext == grid._rows) { max = grid._rows._prev; }
		}
	...
	_showCols(): void {
		...
		if (min == max)
		{
			if (min._visiblePrev == grid._cols) { min = grid._cols._next; }
			if (max._visibleNext == grid._cols) { max = grid._cols._prev; }
		}
	...
	_reIndex<T extends HasIndex>(elt: HiddenList<T>, sentinel: HiddenList<T>): void {
		
		var grid = this;
		
		if (elt == sentinel) { elt = elt._next; }
		if (elt._prev == sentinel) { elt._data._index = 0; elt = elt._next; }
	...
	_setSort(sorter: SortParams): void {
	...
	_applySort(): void {
		...
		var objects = [];
		for (var i = 0; i < rows.length; i++) { objects.push(rows[i]._object); }
